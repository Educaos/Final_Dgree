...
OUTPUTS generados en carpeta "output/":
    ...
    04_por_pais_continente.csv       → Publicaciones por país y continente
    05_por_revista.csv               → Publicaciones por revista / base de datos
    06_revision_vs_investigacion.csv → Revisiones (antecedentes) vs originales
    07_antibioticos_adsorbentes.csv  → Análisis por analito, autor y método
    08_variables_fisicoquimicas.csv  → pHpzc, BET, grupos funcionales, pH op., etc.
    09_datos_cuantitativos.csv       → qmax, % remoción, isoterma, cinética
    10_bibliometrico.csv             → Año, país, revista, adsorbente (incluidos)
=============================================================================
"""
...
# ─────────────────────────────────────────────────────────────────────────────
RIS_FILE   = "F_biblio_zotero_29131_afterclean.ris"  #  nombre de tu archivo RIS exportado desde Zotero
OUTPUT_DIR = "output"
YEAR_MIN   = 2020
YEAR_MAX   = 2025
# ─────────────────────────────────────────────────────────────────────────────
...
separador("SCREENING — BIOADSORBENTES / CONTAMINANTES EMERGENTES")

# ─────────────────────────────────────────────────────────────────────────────
# FUNCIÓN DE NORMALIZACIÓN (única en todo el script)
# ─────────────────────────────────────────────────────────────────────────────
def norm(x) -> str:
...
# ─────────────────────────────────────────────────────────────────────────────
# PASO 2 — NORMALIZAR COLUMNAS CLAVE
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# PASO 3 — DEDUPLICACIÓN RÁPIDA (TF-IDF, no el loop lento de fuzzy)
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{seg()} PASO 3/9 — Detectando duplicados (TF-IDF, ~1-3 min)...")
...
# 3B — Título similar con TF-IDF (100x más rápido que el loop fuzzy)
df["dup_fuzzy"] = False
n_dup_fuzzy = 0
try:
    ...
    dup_idx = set()
    anos = sorted(cand["year_bucket"].unique())

    for ano in tqdm(anos, desc="      Comparando titulos por anno", ncols=65):
...
KW_ADSORCION = [
    ...
    "biochar","activated carbon","carbon activado","carbón activado",
...
RX_OBJ = rx(KW_OBJETIVO)
RX_AGU = rx(KW_AGUA)
RX_ADS = rx(KW_ADSORCION)
RX_EXC_C = rx(KW_EXCL_CONTAM)
RX_EXC_E = rx(KW_EXCL_ESTUDIO)
RX_EXC_D = rx(KW_EXCL_DOC)
...
# ─────────────────────────────────────────────────────────────────────────────
# SUBCONJUNTO DE ARTÍCULOS INCLUIDOS
# ─────────────────────────────────────────────────────────────────────────────
...
# Mapa país → continente
CONTINENTE = {
    # América del Norte
    "united states":"América del Norte","canada":"América del Norte","mexico":"América del Norte",
    # América del Sur / Central
    "brazil":"América del Sur","colombia":"América del Sur","argentina":"América del Sur",
    ...
    "uruguay":"América del Sur","cuba":"América Central/Caribe","costa rica":"América Central/Caribe",
    # Europa
    ...
    # África
    "egypt":"África","nigeria":"África","south africa":"África","morocco":"África",
    "ethiopia":"África","ghana":"África","kenya":"África","algeria":"África",
    # Oceanía
...
ALIAS_P = {
    ...
    "brasil":"brazil","mexico":"mexico","espana":"spain",
...
for df_ in [df_base, df_inc]:
    df_["texto_afil"] = df_.apply(txt_afil, axis=1)
    df_["pais"]       = df_["texto_afil"].apply(detectar_pais)
    df_["continente"] = df_["pais"].map(CONTINENTE).fillna("Sin determinar")

print(f"  OK  Pais detectado en: {df_inc['pais'].notna().sum():,} articulos incluidos")
paises_top = df_inc["pais"].value_counts().head(10)
print("      Top 10 paises (incluidos):")
for p, n in paises_top.items():
    print(f"        {str(p):<30} : {n:>4}")
...
df_base["adsorbente"] = df_base["texto"].apply(detect_ads)
df_inc["adsorbente"]  = df_inc["texto"].apply(detect_ads)

ads_top = df_inc["adsorbente"].value_counts()
print("  OK  Adsorbentes en articulos incluidos:")
for a, n in ads_top.items():
    print(f"        {str(a):<22} : {n:>4}")

# ─────────────────────────────────────────────────────────────────────────────
# PASO 7 — TIPO DE ARTÍCULO: REVISIÓN vs INVESTIGACIÓN ORIGINAL
# ─────────────────────────────────────────────────────────────────────────────
